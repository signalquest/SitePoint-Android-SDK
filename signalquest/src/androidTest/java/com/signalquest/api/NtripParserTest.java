...
public class NtripParserTest {
    ...
    @Test public void testReceiver() throws NtripParser.AuthorizationFailure {
        NtripParser parser = new NtripParser();
        parser.parseAuthorized(icy200Ok);
        parser.parseRtcm(combinedInput);
        // not using circularity, just a convenient container; much larger than needed
        CircularByteBuffer out = new CircularByteBuffer(65536);
        int fetchSize = 8;
        for (byte[] bytes = parser.next(fetchSize); 0 < bytes.length; bytes = parser.next(fetchSize)) {
            out.put(bytes);
        }
        // only testing all data round-tripped, to start
        assertArrayEquals(combinedInput, out.get(65536));
    }
    ...
    byte[] combinedInput;
    {
        final int lengthOfInputMessages = Arrays.stream(inputMessages).map(a -> a.length).mapToInt(Integer::intValue).sum();
        combinedInput = new byte[lengthOfInputMessages];
        int destStart = 0;
